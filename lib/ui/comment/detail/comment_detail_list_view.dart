...
class CommentDetailListView extends StatefulWidget {
  ...
  final DocumentSnapshot commentsS;
                      ...
                      child: Container(
                        ...
                        child: StreamBuilder<QuerySnapshot>(
                            stream: Firestore.instance
                                .collection('comments')
                                .document(widget.commentsS.documentID)
                                .collection('comments')
                                .snapshots(),
                            builder: (context, snapshot) {
                              ...
                              return CustomScrollView(
                                  ...
                                  slivers: <Widget>[
                                    CommentListWidget(
                                        idComments: snapshot.data.documents,
                                        singleComments: widget.commentsS,
                                        animationController:
                                            animationController,
                                        provider: provider),
                                  ]);
                            ...
                            if (commentController.text.isEmpty) {
                              ...
                              if (await utilsCheckInternetConnectivity()) {
                                utilsNavigateOnUserVerificationView(context,
                                    () async {
                                  sl.get<FirebaseBloc>().commentInIt({
                                    'productName':
                                        widget.comments['ProductName'],
                                    'reference': widget.comments['Reference'],
                                    'uid': users.uid,
                                    'image': snapshot.data.data['ProfileImage'],
                                    'createdtime': Timestamp.now(),
                                    'name': snapshot.data.data['name'],
                                    'Comments': commentController.text
                                  }, widget.comments.documentID);
                                });
                              } else {
                                showDialog<dynamic>(
                                    context: context,
...
class CommentListWidget extends StatefulWidget {
  const CommentListWidget({
    ...
    this.provider,
    @required this.singleComments,
    @required this.idComments,
  }) : super(key: key);

  final AnimationController animationController;
  final CommentDetailProvider provider;
  final DocumentSnapshot singleComments;
  final List<DocumentSnapshot> idComments;
  @override
    ...
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (BuildContext context, int index) {
          ...
          final int count = widget.idComments.length;
          return CommetDetailListItemView(
            comments: widget.idComments[index],
        ...
        childCount: widget.idComments.length,
      ),
    );
  }
}
